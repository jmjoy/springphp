...
class DateEditor extends PropertyEditor {

	var $dateFormat = 'Y-m-d H:i:s';
	var $allowEmpty = false;

	function DateEditor($dateFormat = null, $allowEmpty = false) {
	...
	function setValue($val) {
		if(is_array($val)) {
			...
			$this->value =& new Date(mktime($time['hour'], $time['minute'], $time['second'], $time['month'], $time['day'], $time['year']));
		}else if(is_a($val, 'Date')){
			$this->value =& $val;
		}else {
	...
	function setAsText($text) {
		if($text==null || strlen(trim($text)) == 0) {
			if ($this->allowEmpty)  $this->setValue(new Date(time()));
			else return false;
		}
		$time = strtotime($text);
	...
	function getAsText() {
		if(!is_null($this->value) && is_a($this->value, 'Date')) return date($this->dateFormat, $this->value->getTime());
