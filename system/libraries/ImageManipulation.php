...
class ImageManipulation {
	...
	{
		log_message(LOG_DEBUG, "Image Manipulation Initialized");
	}
	...
	function thumbnail()
	{
		...
		return $this->$protocol('thumbnail');
	}
	// --------------------------------------------------------------------
	...
	function image_process_imagemagick($action = 'resize')
	{
		...
		{
			if ( ! preg_match("#[\.jpg|\.jpeg|\.gif|\.png]$#i", $this->full_src_path))
				$this->full_src_path = $this->full_src_path.'[0]';
		
			$cmd .= " -colorspace RGB -thumbnail ".$this->width."x".$this->height." \"$this->full_src_path\" \"$this->full_dst_path\" 2>&1";		
		}
		else  // Resize
		{
			if ( ! preg_match("#[\.jpg|\.jpeg|\.gif|\.png]$#i", $this->full_src_path))
			...
			$cmd .= " -colorspace RGB -resize ".$this->width."x".$this->height." \"$this->full_src_path\" \"$this->full_dst_path\" 2>&1";
		}
	...
	function set_error($msg)
	{
		...
		if (is_array($msg))
		{
			foreach ($msg as $val)
			{
				...
				$msg = !isset($lang[$val]) ? $val : $lang[$val];
				$this->error_msg[] = $msg;
		...
		{
			$msg = !isset($lang[$msg]) ? $msg : $lang[$msg];
			$this->error_msg[] = $msg;
			log_message(LOG_ERR, $msg);
		}
	}
