 ...
 *
 * @package		Redstart
 * @subpackage  Vendors-CodeIgniter
 * @author		Rick Ellis
...
class CI_DB_driver {
	...
	*/
	...
	function CI_DB_driver($params)
	{
		...
		log_message(LOG_DEBUG, 'Database Driver Class Initialized');
	}
...
//								'cache_on'	=> FALSE
		...
		elseif (strpos($params, '://'))
		{
			...
			{
				log_message(LOG_ERR, 'Invalid DB Connection String');
		...
		if ( ! $this->conn_id)
		{
			log_message(LOG_ERR, 'Unable to connect to the database');
		...
		if ($this->database != '')
		{
			if ( ! $this->db_select())
			{
				log_message(LOG_ERR, 'Unable to select database: '.$this->database);
	...
	function query($sql, $binds = FALSE, $return_object = TRUE)
	{
		if ($sql == '')
		{
			if ($this->db_debug)
			{
				log_message(LOG_ERR, 'Invalid query: '.$sql);
		...
		*/
		...
		if (FALSE === ($this->result_id = $this->simple_query($sql)))
		{
			...
			if ($this->db_debug)
			{
				log_message(LOG_ERR, 'Query error: '.$this->_error_message());
	...
	function load_rdriver()
	{
		...
		{
			include_once(BASEPATH.'vendors/database/DB_result'.EXT);
			include_once(BASEPATH.'vendors/database/drivers/'.$this->dbdriver.'/'.$this->dbdriver.'_result'.EXT);
		}
...
*/
	...
	function display_error($error = '', $swap = '', $native = FALSE)
	{
		...
		if($native == TRUE)
			show_error('A DB Error Was Encountered', $error);
		else{
			...
			show_error('A DB Error Was Encountered', $message);
